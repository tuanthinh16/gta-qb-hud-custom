...
local function getVehicleMaxRpm(vehicle)
    if not DoesEntityExist(vehicle) then return 7000 end
    
    -- Lấy giá trị maxRpm từ handling hoặc dùng default
    local maxRpm = GetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveMaxFlatVel") * 100
    return math.floor(math.min(maxRpm, 12000)) -- Giới hạn tối đa 12000 RPM
end
local function updateVehicleHud(data)
    ...
    if DoesEntityExist(vehicle) then
        ...
        maxRpm = getVehicleMaxRpm(vehicle)
        
        -- Chuẩn hóa RPM theo maxRpm thực tế (0.0-1.0)
        data.rpmNormalized = rpm 
        data.rpmAngle = rpm * 240 - 120
        -- Lấy name và gear
        local vehicleModel = GetEntityModel(vehicle)
        local vehicleName = GetDisplayNameFromVehicleModel(vehicleModel)
        local vehicleLabel = GetLabelText(vehicleName)
        data.vehicleName = vehicleLabel ~= "NULL" and vehicleLabel or vehicleName
        ...
        elseif gear == -1 then
            ...
            data.vehicleGear = tostring(gear) 
        end
    end

    -- Kiểm tra thay đổi (bao gồm cả RPM)
    local shouldUpdate = false
    for k, v in pairs(data) do
    ...
    if shouldUpdate then
        ...
        prevVehicleStats = table.clone(data) -- Sao chép bảng để so sánh lần sau
        -- print(string.format("[RPM] Current: %.2f | Max: %d | Angle: %.1f°", 
        --       rpm, maxRpm, data.rpmAngle or 0))
        SendNUIMessage({
            action = 'car',
            show = data[1],
            isPaused = data[2],
            ...
            showSquareB = data[9],
            showCircleB = data[10],
            rpm = data.rpmNormalized or 0.0,
            maxRpm = maxRpm,
            rpmAngle = data.rpmAngle or -120,
            vehicleName = data.vehicleName or "",
            gear = data.vehicleGear or "P"
        })
    end
end

-- Hàm sao chép bảng (nếu chưa có)
